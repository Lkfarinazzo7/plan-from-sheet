...
export function ExcelImportDialog({ open, onOpenChange, title, expectedColumns, mapRow, onConfirm }: ExcelImportDialogProps) {
  ...
  const handleConfirm = async () => {
    ...
    setImporting(true);
    try {
      await onConfirm(validRows.map(r => r.data));
          ...
          <div className="space-y-3 flex-1 min-h-0 flex flex-col">
            <div className="flex items-center gap-4 text-sm">
              <span className="flex items-center gap-1 text-success">
                <CheckCircle className="h-4 w-4" /> {validRows.length} válidos
              </span>
              {errorRows.length > 0 && (
                <span className="flex items-center gap-1 text-destructive">
                  <AlertTriangle className="h-4 w-4" /> {errorRows.length} com erros
                </span>
              )}
            </div>
                ...
                <TableBody>
                  {parsedRows.slice(0, 50).map((row, i) => (
                    <TableRow key={i} className={row.errors.length > 0 ? 'bg-destructive/5' : ''}>
                      {rawHeaders.map(h => {
                        const originalRow = XLSX.utils.sheet_to_json<Record<string, any>>(
                          XLSX.read(undefined)?.Sheets?.[''] || {}
                        );
                        return <TableCell key={h} className="text-xs">{String(row.data[`_raw_${h}`] ?? row.data[h] ?? '')}</TableCell>;
                      })}
                      <TableCell>
                        {row.errors.length > 0 ? (
            ...
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => { setParsedRows([]); setFileName(''); }}>
